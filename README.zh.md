...
> **关于 registry：** 目前 KPM 没有中心化的官方 registry，欢迎有意愿的机构自行搭建并开放给社区使用。如果你是希望分享知识的个人，可以直接将打包好的 `.tar.gz` 文件托管到任意可公开访问的 URL（如 GitHub），然后通过 `kpm add` 直接引用，例如：
> ```
...
| 前缀 | 示例 | 含义 |
|---|---|---|
| 无（裸名称） | `"^1.0.0"` | 通过 registries 解析。**需要配置 `registries` 字段或 `KPM_REGISTRY` 环境变量。** |
...
| 文件 | 说明 |
|---|---|
| `docs/sphinx-to-markdown.md` | 如何将 Python 包的 Sphinx 文档转换为 Markdown，并打包为 KPM 知识包 |
